        ...
        if ($this->link) {
            ...
            foreach ($polygon->all() as $point) {
                ...
                if ($prev !== NULL and $point !== NULL) {
                    $driver->line(
                            $this->lineColor, new awLine(
                            $prev, $point, $this->lineStyle, $this->lineThickness
                            )
                    );
                }
                $prev = $point;
            ...
            foreach ($polygon->all() as $key => $point) {
                ...
                if ($point !== NULL) {
                    ...
                    $zero = awAxis::toPosition(
                                    $this->xAxis, $this->yAxis, new awPoint($key, 0)
                    );

                    $driver->line(
                            $this->impulse, new awLine(
                            $zero, $point, awLine::SOLID, 1
                            )
                    );
                }
            }
