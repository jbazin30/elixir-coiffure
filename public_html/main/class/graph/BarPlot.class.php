    ...
    public function drawComponent(awDriver $driver, $x1, $y1, $x2, $y2, $aliasing) {
        ...
        for ($key = 0; $key < $count; $key++) {
            ...
            if ($value !== NULL) {
                ...
                $p1 = new awPoint(
                        round($barStart) + $this->depth + $this->move->left, round($t1) - $this->depth + $this->move->top
                );

                $p2 = new awPoint(
                        round($barStop) + $this->depth + $this->move->left, round($t2) - $this->depth + $this->move->top
                );
        ...
        foreach ($this->datay as $key => $value) {
            ...
            if ($value !== NULL) {
                ...
                $point = new awPoint(
                        $barPosition + ($this->identifier - 1) * $this->barSpace + $position->x + $barSize / 2 + 1 + $this->depth, $position->y - $this->depth
                );
