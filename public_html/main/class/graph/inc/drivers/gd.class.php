    ...
    public function init(awImage $image) {
        ...
        if ($this->resource === NULL) {
            ...
            $this->filledRectangle(
                    ...
                    new awPoint(0, 0), new awPoint($this->imageWidth, $this->imageHeight)
                    )
            );
        ...
        if ($line->thickness > 0 and $line->isHidden() === FALSE) {
            ...
            switch ($line->getStyle()) {
                ...
                case awLine::DOTTED :
                    ...
                    for ($i = 0; $i <= $size; $i += 2) {
                        $p = new awPoint(
                                round($i * $cos + $p1->x), round($i * $sin + $p1->y)
                        );
                        $this->point($color, $p);
                    }
                    break;
                ...
                case awLine::DASHED :
                    ...
                    for ($i = 0; $i <= $size; $i += 6) {
                        ...
                        $t2 = new awPoint(
                                round($functionX(($i + 3) * $cos, $width) + $p1->x), round($functionY(($i + 3) * $sin, $height) + $p1->y)
                        );
...
class awGDGradientDriver {
    ...
    private function drawFilledTriangleVertically(awGradient $gradient, awPolygon $polygon) {
        ...
        for ($y = round($yMin); $y < round($yMax); $y++) {
            ...
            if (count($toX) === 1) {
                $fillLine = new Line(
                        new Point($fromX, $y), new Point($toX[0], $y)
                );
            } else {
                ...
                $line2 = new Line(
                        new Point($fromX, $y), new Point($toX[1], $y)
                );
    ...
    private function drawFilledTriangleHorizontally(awGradient $gradient, awPolygon $polygon) {
        ...
        for ($x = round($xMin); $x < round($xMax); $x++) {
            ...
            if (count($toY) === 1) {
                $fillLine = new Line(
                        new Point($x, $fromY), new Point($x, $toY[0])
                );
            } else {
                ...
                $line2 = new Line(
                        new Point($x, $fromY), new Point($x, $toY[1])
                );
        ...
        if ($y1 - $y2 > 0) {
            ...
            for ($i = -$r; $i <= $r; $i++) {
                ...
                if ($gradient->angle === 90) {
                    ...
                    // Print the line
                    $p1 = new awPoint(
                            $x1 + $i + $r, round(max($y2 + $r - $h + 1, $y2))
                    );

                    $p2 = new awPoint(
                            $x1 + $i + $r, round(min($y1 - $r + $h - 1, $y1))
                    );
                } else {
                ...
                for ($i = 2; $i < $count - 1; $i++) {
                    ...
                    if ($current->x - $prev->x > 0) {
                        ...
                        $gradient = new awLinearGradient(
                                $this->color($max - $min - ($y2 - $y1)), $this->color($max - $min), 0
                        );
                        ...
                        $gradient = new awLinearGradient(
                                $this->color($bottom->y - $min), $this->color($max - $min - ($y2 - $y1)), 0
                        );
    ...
    private function color($pos) {
        ...
        return new awColor(
                $this->getRed($pos), $this->getGreen($pos), $this->getBlue($pos), $this->getAlpha($pos)
        );
    }
