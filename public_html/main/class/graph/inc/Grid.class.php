...
class awGrid {
    ...
    public function draw(awDriver $driver, $x1, $y1, $x2, $y2) {
        ...
        if ($this->hide === FALSE) {
            ...
            $this->drawGrid(
                    $driver, $this->color, $this->hideVertical ? [] : $this->xgrid, $this->hideHorizontal ? [] : $this->ygrid, $x1, $y1, $x2, $y2, $this->type, $this->space, $this->interval[0], $this->interval[1]
            );
        }
    }
    ...
    awDriver $driver, awColor $color, $nx, $ny, $x1, $y1, $x2, $y2, $type, $space, $hInterval, $vInterval
        ...
        foreach ($nx as $key => $n) {
            ...
            if (($key % $vInterval) === 0) {
                ...
                $driver->line(
                        $color, new awLine(
                        new awPoint($pos, $y1), new awPoint($pos, $y2), $type
                        )
                );
            }
        }
        ...
        foreach ($ny as $key => $n) {
            ...
            if (($key % $hInterval) === 0) {
                ...
                $driver->line(
                        $color, new awLine(
                        new awPoint($x1, $pos), new awPoint($x2, $pos), $type
                        )
                );
            }
        }
