...
if (!defined('__CLASS_HTML2PDF__')) {
    ...
    class HTML2PDF {
        ...
        protected $background = []; // informations sur le background
        ...
        protected $maxSave = []; // tableau de sauvegarde des maximaux
        ...
        protected $marges = []; // tableau de sauvegarde des differents etats des marges de la page courante
        protected $inLink = '';  // indique si on est � l'interieur d'un lien
        protected $lstAncre = []; // liste des ancres d�tect�es ou cr��es
        protected $subHEADER = []; // tableau des sous commandes pour faire l'HEADER
        protected $subFOOTER = []; // tableau des sous commandes pour faire le FOOTER
        protected $subSTATES = []; // tableau de sauvegarde de certains param�tres
        protected $defLIST = []; // tableau de sauvegarde de l'etat des UL et OL
        protected $lstChamps = []; // liste des champs
        protected $lstSelect = []; // options du select en cours
        protected $previousCall = null;  // dernier appel
        protected $pageMarges = []; // marges sp�cifiques dues aux floats
        ...
        static protected $TABLES = []; // tableau global necessaire � la gestion des tables imbriqu�es
        static protected $TEXTES = []; // tableau comprennant le fichier de langue
        ...
        protected function DEBUG_add($nom, $level = null) {
            ...
            $this->DEBUG_stepline(
                    $nom, number_format(($time - $this->DEBUG_start_time) * 1000, 1, '.', ' ') . ' ms', number_format(($time - $this->DEBUG_last_time) * 1000, 1, '.', ' ') . ' ms', number_format($usage / 1024, 1, '.', ' ') . ' Ko', number_format($peak / 1024, 1, '.', ' ') . ' Ko');
...
//				$this->pdf->setWordSpacing(0);
        ...
        protected function prepareSubObj() {
            ...
            HTML2PDF::$SUBOBJ = new HTML2PDF(
                    $this->sens, $this->format, $this->langue, $this->unicode, $this->encoding, array($this->defaultLeft, $this->defaultTop, $this->defaultRight, $this->defaultBottom)
            );
...
//			if (($w==0 && $this->style->value['width']==0) || ($w>$this->style->value['width']) || $this->style->value['position']=='absolute')
            ...
            $this->drawRectangle(
                    $this->style->value['x'], $this->style->value['y'], $this->style->value['width'], $this->style->value['height'], $this->style->value['border'], $this->style->value['padding'], 0, $this->style->value['background']
            );
            ...
            if (!$this->sub_part && !$this->isSubPart) {
                switch ($this->style->value['text-align']) {
                    case 'right': $x_corr = ($this->style->value['width'] - $w_reel);
                        break;
                    case 'center': $x_corr = ($this->style->value['width'] - $w_reel) * 0.5;
                        break;
                }
                if ($x_corr > 0)
                    $x_corr = 0;
                switch ($this->style->value['vertical-align']) {
                    case 'bottom': $y_corr = ($this->style->value['height'] - $h_reel);
                        break;
                    case 'middle': $y_corr = ($this->style->value['height'] - $h_reel) * 0.5;
                        break;
                }
            }
            ...
            if ($overflow) {
                ...
                $this->pdf->clippingPathOpen(
                        $this->style->value['x'] + $marge['l'], $this->style->value['y'] + $marge['t'], $this->style->value['width'], $this->style->value['height']
                );
        ...
        protected function o_WRITE($param) {
            ...
            if ($this->style->value['text-transform'] != 'none') {
                if ($this->style->value['text-transform'] == 'capitalize')
                    $txt = ucwords($txt);
                elseif ($this->style->value['text-transform'] == 'uppercase')
                    $txt = strtoupper($txt);
                elseif ($this->style->value['text-transform'] == 'lowercase')
                    $txt = strtolower($txt);
            }
        ...
        protected function Image($src, $sub_li = false) {
            ...
            if ($this->style->value['width'] && $this->style->value['height']) {
                $w = $this->style->value['width'];
                $h = $this->style->value['height'];
            } elseif ($this->style->value['width']) {
                $w = $this->style->value['width'];
                $h = $hi * $w / $wi;
            } elseif ($this->style->value['height']) {
                $h = $this->style->value['height'];
                $w = $wi * $h / $hi;
            } else {
                $w = 72. / 96. * $wi;
...
//				$this->maxX = max($this->maxX, $x+$w);
        ...
        protected function drawRectangle($x, $y, $w, $h, $border, $padding, $margin, $background) {
            ...
            if ($background['image']) {
                ...
                if (count($i_infos) < 2) {
                    ...
                    if ($i_repeat[0])
                        $i_position[0] = $b_x;
                    elseif (preg_match('/^([-]?[0-9\.]+)%/isU', $i_position[0], $match))
                    ...
                    if ($i_repeat[1])
                        $i_position[1] = $b_y;
                    elseif (preg_match('/^([-]?[0-9\.]+)%/isU', $i_position[1], $match))
        ...
        protected function o_THEAD($param) {
            ...
            if ($this->sub_part) {
                ...
                HTML2PDF::$TABLES[$param['num']]['thead']['code'] = [];
        ...
        protected function o_TFOOT($param) {
            ...
            if ($this->sub_part) {
                ...
                HTML2PDF::$TABLES[$param['num']]['tfoot']['code'] = [];
                for ($pos = $this->temp_pos; $pos < count($this->parsing->code); $pos++) {
                    $todo = $this->parsing->code[$pos];
        ...
        protected function o_TABLE($param, $other = 'table') {
            ...
            if (isset($param['align']))
                unset($param['align']);
            if (!in_array($align_object, array('left', 'center', 'right')))
                $align_object = 'left';
            ...
            if ($this->sub_part) {
                ...
                HTML2PDF::$TABLES[$param['num']]['cases'] = []; // liste des propri�t�s des cases
                HTML2PDF::$TABLES[$param['num']]['corr'] = []; // tableau de correlation pour les colspan et rowspan
                ...
                HTML2PDF::$TABLES[$param['num']]['marge'] = [];
                ...
                HTML2PDF::$TABLES[$param['num']]['thead'] = []; // infos sur le thead
                HTML2PDF::$TABLES[$param['num']]['tfoot'] = []; // infos sur le tfoot
                HTML2PDF::$TABLES[$param['num']]['thead']['tr'] = []; // tr du thead
                HTML2PDF::$TABLES[$param['num']]['tfoot']['tr'] = []; // tr du tfoot
                HTML2PDF::$TABLES[$param['num']]['thead']['height'] = 0; // hauteur du thead
                HTML2PDF::$TABLES[$param['num']]['tfoot']['height'] = 0; // hauteur du tfoot
                HTML2PDF::$TABLES[$param['num']]['thead']['code'] = [];   // contenu HTML du thead
                HTML2PDF::$TABLES[$param['num']]['tfoot']['code'] = [];   // contenu HTML du tfoot
                HTML2PDF::$TABLES[$param['num']]['cols'] = []; // definition via les balises col
                ...
                $this->drawRectangle(
                        HTML2PDF::$TABLES[$param['num']]['curr_x'], HTML2PDF::$TABLES[$param['num']]['curr_y'], HTML2PDF::$TABLES[$param['num']]['width'], isset(HTML2PDF::$TABLES[$param['num']]['height'][0]) ? HTML2PDF::$TABLES[$param['num']]['height'][0] : null, $this->style->value['border'], $this->style->value['padding'], 0, $this->style->value['background']
                );
            ...
            if ($this->sub_part) {
                ...
                if ($parent_w > $w) {
                    if (HTML2PDF::$TABLES[$param['num']]['align'] == 'center')
                        $x = $x + ($parent_w - $w) * 0.5;
                    elseif (HTML2PDF::$TABLES[$param['num']]['align'] == 'right')
                        $x = $x + $parent_w - $w;
                ...
                HTML2PDF::$TABLES[$param['num']]['height'] = [];
                ...
                for ($k = 0; $k < count(HTML2PDF::$TABLES[$param['num']]['cases']); $k++) {
                    ...
                    if (in_array($k, HTML2PDF::$TABLES[$param['num']]['thead']['tr']))
                        continue;
                    if (in_array($k, HTML2PDF::$TABLES[$param['num']]['tfoot']['tr']))
                        continue;
                ...
                elseif (count(HTML2PDF::$TABLES[$param['num']]['height']) == 1)
            ...
            if (!$this->sub_part) {
                ...
                if (!$this->isInTfoot && HTML2PDF::$TABLES[$param['num']]['td_y'] + HTML2PDF::$TABLES[$param['num']]['marge']['b'] + $ty + $hfoot > $this->pdf->getH() - $this->pdf->getbMargin()) {
                    ...
                    if (isset(HTML2PDF::$TABLES[$param['num']]['height'][HTML2PDF::$TABLES[$param['num']]['page']])) {
                        ...
                        // initialisation du style des bordures de la premiere partie de tableau
                        $this->drawRectangle(
                                HTML2PDF::$TABLES[$param['num']]['curr_x'], HTML2PDF::$TABLES[$param['num']]['curr_y'], HTML2PDF::$TABLES[$param['num']]['width'], HTML2PDF::$TABLES[$param['num']]['height'][HTML2PDF::$TABLES[$param['num']]['page']], $this->style->value['border'], $this->style->value['padding'], HTML2PDF::$TABLES[$param['num']]['cellspacing'] * 0.5, $this->style->value['background']
                        );
                ...
                HTML2PDF::$TABLES[$param['num']]['cases'][HTML2PDF::$TABLES[$param['num']]['tr_curr'] - 1] = [];
                if (!isset(HTML2PDF::$TABLES[$param['num']]['corr'][HTML2PDF::$TABLES[$param['num']]['corr_y']]))
                    HTML2PDF::$TABLES[$param['num']]['corr'][HTML2PDF::$TABLES[$param['num']]['corr_y']] = [];
        ...
        protected function o_TD($param, $other = 'td') {
            ...
            if ($this->sub_part) {
                ...
                HTML2PDF::$TABLES[$param['num']]['cases'][$y][$x] = [];
                ...
                // initialisation du style des bordures de la premiere partie de tableau
                $this->drawRectangle(
                        HTML2PDF::$TABLES[$param['num']]['td_x'], HTML2PDF::$TABLES[$param['num']]['td_y'], HTML2PDF::$TABLES[$param['num']]['cases'][$y][$x]['w'], HTML2PDF::$TABLES[$param['num']]['cases'][$y][$x]['h'], $this->style->value['border'], $this->style->value['padding'], HTML2PDF::$TABLES[$param['num']]['cellspacing'] * 0.5, $this->style->value['background']
                );
        ...
        protected function o_INPUT($param) {
            ...
            switch ($param['type']) {
                case 'checkbox':
                    ...
                    if ($h < $f)
                        $y+= ( $f - $h) * 0.5;
                    $this->pdf->CheckBox($name, $w, isset($param['checked']), [], [], ($param['value'] ? $param['value'] : 'Yes'), $x, $y);
                    break;
                ...
                case 'radio':
                    ...
                    if ($h < $f)
                        $y+= ( $f - $h) * 0.5;
                    $this->pdf->RadioButton($name, $w, [], [], ($param['value'] ? $param['value'] : 'On'), isset($param['selected']), $x, $y);
                    break;
                ...
                case 'submit':
                    ...
                    $this->pdf->Button($name, $w, $h, $param['value'], $action, [], [], $x, $y);
                    break;
