...
class styleHTML {
    ...
    protected $htmlColor = [];  // liste des couleurs HTML
    ...
    public $value = [];  // valeurs actuelles
    public $css = [];  // tableau des CSS
    public $css_keys = [];  // tableau des clefs CSS, pour l'ordre d'execution
    public $table = [];  // tableau d'empilement pour historisation des niveaux
    ...
    public function resetStyle($balise = '') {
        ...
        if (!in_array($balise, array('tr', 'td', 'th', 'thead', 'tbody', 'tfoot')))
            $collapse = false;
            ...
            switch ($nom) {
                ...
                case 'border-radius':
                    ...
                    if (!isset($val_h[1]))
                        $val_h[1] = $val_h[0];
                    if (!isset($val_h[2]))
                        $val_h = array($val_h[0], $val_h[0], $val_h[1], $val_h[1]);
                    if (!isset($val_h[3]))
                        $val_h[3] = $val_h[1];
                    ...
                    if (isset($val[1])) {
                        ...
                        if (!isset($val_v[1]))
                            $val_v[1] = $val_v[0];
                        if (!isset($val_v[2]))
                            $val_v = array($val_v[0], $val_v[0], $val_v[1], $val_v[1]);
                        if (!isset($val_v[3]))
                            $val_v[3] = $val_v[1];
                    } else
                        $val_v = $val_h;
     ...
     *
     * @return	[]		tableau des propri�t�s CSS
     */
    protected function getFromCSS() {
        $styles = []; // style � appliquer
        $getit = []; // styles � r�cuperer
     ...
     * @param	[]		tableau des styles direct, et ceux des parents
    ...
    public function readBorder($val) {
        ...
        foreach ($val as $key) {
            ...
            elseif (in_array($key, array('solid', 'dotted', 'dashed', 'double')))
                $type = $key;
            else {
    ...
    public function ConvertBackgroundImage($val) {
        if ($val == 'none')
            return null;
        elseif (preg_match('/^url\(([^)]*)\)$/isU', $val, $match))
            return $match[1];
        else
            return null;
    }
    ...
    public function ConvertBackgroundPosition($val, &$res) {
        ...
        elseif ($val[0] == 'bottom')
            $y = '100%';
        elseif (preg_match('/^[-]?[0-9\.]+%$/isU', $val[0]))
            $x = $val[0];
        elseif ($this->ConvertToMM($val[0]))
        ...
        elseif ($val[1] == 'bottom')
            $y = '100%';
        elseif (preg_match('/^[-]?[0-9\.]+%$/isU', $val[1]))
            $y = $val[1];
        elseif ($this->ConvertToMM($val[1]))
    ...
    public function ConvertToMM($val, $old = 0.) {
        ...
        elseif (preg_match('/^[0-9\.\-]+in$/isU', $val))
            $val = 25.4 * str_replace('in', '', $val);
        elseif (preg_match('/^[0-9\.\-]+mm$/isU', $val))
